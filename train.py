...
class ShapeNetSplatsDataset(Dataset):
    ...
    def __init__(self, data_dir=None, num_views=24, limit=None):
        ...
        if data_dir is None:
            data_dir = Path("/home/john.noe/projects/gaussian_splats_vae/data/aligned")
...
class SimpleGaussianRenderer(nn.Module):
    ...
    def forward(self, splats):
        ...
        rendered = torch.zeros(B, 1, self.size, self.size, device=splats.device)
...
def vae_loss(rendered, target_views, mu, logvar, splats, beta_kl=1.0, 
    ...
    kl_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) / rendered.shape[0]
    
    radii = splats[..., 3]
    size_reg = lambda_size * torch.sum(radii ** 2) / rendered.shape[0]
    
    opacity = splats[..., 5]
    opacity_reg = lambda_opacity * torch.sum(torch.sigmoid(opacity)) / rendered.shape[0]
...
def train_vae(model, train_loader, val_loader, epochs=100, lr=1e-3, device='cuda'):
    ...
    for epoch in range(epochs):
        ...
        for views in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} [Train]"):
            ...
            loss, l1, kl = vae_loss(rendered, views, mu, logvar, splats, beta_kl=beta_kl)
        ...
        with torch.no_grad():
            for views in tqdm(val_loader, desc=f"Epoch {epoch+1}/{epochs} [Val]"):
                views = views.to(device)
                rendered, mu, logvar, splats = model(views)
                loss, _, _ = vae_loss(rendered, views, mu, logvar, splats, beta_kl=1.0)
        ...
        print(f"Epoch {epoch+1}/{epochs} | Train: {train_loss:.4f} | Val: {val_loss:.4f} | PSNR: {val_psnr:.2f} | β_KL: {beta_kl:.3f}")
...
if __name__ == '__main__':
    ...
    # Create datasets
    print("Loading data...")
    train_dataset = ShapeNetSplatsDataset(limit=1000)
    ...
    model, logs = train_vae(model, train_loader, val_loader, epochs=100, lr=1e-3, device=device)
    ...
    print("\n✓ Complete!")
    print("Saved: vae_final.pt, training_logs.json")